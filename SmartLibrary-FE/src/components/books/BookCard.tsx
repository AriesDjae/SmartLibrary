...
const BookCard: React.FC<BookCardProps> = ({ book, size = 'medium' }) => {
  ...
  return (
    <motion.div 
      className={`card group book-card-transition ${cardClasses[size]}`}
      whileHover={{ y: -5 }}
      ...
      <Link to={`/books/${id}`} className="block h-full">
        <div className="relative h-full flex flex-col">
          ...
          <div className="p-3 flex-grow flex flex-col justify-between">
            <div>
              <h3 className={`font-medium line-clamp-2 ${titleClasses[size]}`}>{title}</h3>
            ...
            <div className="mt-2 flex items-center justify-between">
              ...
              <div className="flex items-center">
                <Star className="h-3.5 w-3.5 text-highlight-500 fill-highlight-500" />
                <span className="text-sm ml-1">{rating}</span>
              </div>
              
              {/* Primary Genre Tag */}
              {genres && genres.length > 0 && (
                <span className="badge-blue text-xs">{genres[0]}</span>
              )}
            </div>
          </div>
        </div>
