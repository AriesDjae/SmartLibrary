...
const AiPage: React.FC = () => {
  ...
  useEffect(() => {
    if (initialPrompt) {
      const newMessages = [
        ...
        {
          role: "ai" as const,
          content: `Ini adalah jawaban AI untuk: "${initialPrompt}"`,
        },
      ];
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    const newMessages = [
      ...
      {
        role: "ai" as const,
        content: `Ini adalah jawaban AI untuk: "${trimmed}"`,
      },
    ];
    setMessages(newMessages);
    setInput("");
    ...
    if (selectedChatId) {
      setHistory((prev) =>
        prev.map((h) =>
          h.id === selectedChatId
            ? { ...h, messages: newMessages, title: generateTitle(newMessages) }
            : h
        )
      );
    } else {
  ...
  const activeTitle =
    history.find((h) => h.id === selectedChatId)?.title || "Percakapan AI";
      ...
      <div className="container mx-auto pt-8 flex-1 flex flex-col max-w-6xl w-full">
        ...
        <div className="flex flex-1 gap-6">
          ...
          <section className="flex-1 flex flex-col">
            ...
            <div
              className="flex-1 overflow-y-auto bg-white rounded-xl shadow-sm border border-gray-100 px-0 sm:px-6 py-4 flex flex-col relative"
              style={{ paddingBottom: "90px", minHeight: 400 }}
            >
              {messages.length === 0 && (
                <div className="flex flex-col items-center justify-center h-full py-16">
                  ...
                  <div className="text-gray-400 text-sm">
                    Tanyakan apa saja tentang buku, penulis, atau topik
                    perpustakaan!
                  </div>
                </div>
              )}
              {messages.map((msg, idx) => (
                ...
                >
                  <div className={`flex items-end gap-2 max-w-[85%]`}>
                    ...
                    <div
                      className={`px-4 py-2 rounded-2xl text-sm whitespace-pre-line break-words shadow-sm ${
                        msg.role === "user"
            ...
            <form
              onSubmit={handleSend}
              className="w-full flex gap-2 px-2 sm:px-0 py-4 bg-gray-50 sticky bottom-0 z-10 mb-8"
